...
format_var_summary_1 <- function(df, var_name) {
  df %>%
    select(`Platform` = 1, `# Observations` = 2,
...
create_density_for_transform <- function(df, title, x_title) {
  ggplot(df, aes(x=x)) +
    geom_density() +
    labs(title = title, x = x_title) +
    theme_pubr(base_size = 10) +
    theme(plot.title = element_text(size=11))
}

log_transform <- function(df, min_x = 0, shift_x = 1) {
  df %>% filter(x > min_x) %>% mutate(x = log(x + shift_x)) %>%
    mutate(x = (x - min(x)) / (max(x) - min(x)) )
}

arcsine_transform <- function(df) {
  df %>% mutate(x = (x - min(x)) / (max(x) - min(x))) %>%
    mutate(x = asin(sqrt(x))) %>%
    mutate(x = (x - min(x)) / (max(x) - min(x)))
}

cube_root_transformation <- function(df, shift_x = 1) {
  df %>% mutate(x = (x+shift_x)^(1/3)) %>%
    mutate(x = (x - min(x)) / (max(x) - min(x)))
}

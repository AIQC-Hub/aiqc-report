...
# that page's .qmd changes. Almost nothing that decides what this page shows
# lives in the .qmd: the code is in reportlib, and every section is a child in
# _func/ with its figures in images/. None of those are hashed by freeze, so a
# stale freeze would quietly publish the previous build's output.
#
...
stamp="$(
  {
    ...
    find content/_func content/images -type f -exec md5sum {} + 2>/dev/null | sort
  } | md5sum | cut -d' ' -f1
)"
if [ -d content/_freeze ] && [ "$(cat "$stamp_file" 2>/dev/null)" != "$stamp" ]; then
  echo "reportlib, _func/ or images/ moved -- clearing content/_freeze"
